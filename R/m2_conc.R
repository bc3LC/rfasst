  ...
  if (!recompute & exists('m2_get_conc_pm25.output')) {
    ...
    for (sc in scen_name) {
      ...
      pm25.write<-function(df){
        ...
        write.csv(df,paste0("output/","m2/","NAT_PRIM_SEC_PM2.5_",sc,"_",unique(df$year),".csv"),row.names = F)
      }
      ...
      pm25.agg.write<-function(df){
        ...
        write.csv(df,paste0("output/","m2/","PM2.5_",sc,"_",unique(df$year),".csv"),row.names = F)
      }
  ...
  if (!recompute & exists('m2_get_conc_o3.output')) {
    ...
    for (sc in scen_name) {
      ...
      o3.write<-function(df){
        ...
        colnames(df)<-c("region","year","units","value")
        write.csv(df,paste0("output/","m2/","O3_",sc,"_",unique(df$year),".csv"),row.names = F)
      }
  ...
  if (!recompute & exists('m2_get_conc_m6m.output')) {
    ...
    for (sc in scen_name) {
      ...
      m6m.write<-function(df){
        df<-as.data.frame(df) %>% dplyr::select(-pollutant)
        colnames(df)<-c("region","year","units","value")
        write.csv(df,paste0("output/","m2/","M6M_",sc,"_",unique(df$year),".csv"),row.names = F)
      }
