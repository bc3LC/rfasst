...
usethis::use_data(pop.all.ctry_nuts3.str.SSP2_w, overwrite = T)
...
for (dd in na.omit(unique(mort.rates_iso_sf$disease))) {
  ...
  save(mort.rates_mat_yy, file = paste0('inst/extdata/tmp/mort.rates_mat_',dd,'.RData'))
  assign(paste0('mort.rates_mat_',dd),mort.rates_mat_yy)
...
GBD_mat <- list()
for (dd in na.omit(unique(GBD_sf$disease))[4:5]) {
  print(dd)
  ...
  assign(paste0('GBD_mat_',dd), GBD_mat_yy)
  obj_name <- paste0('GBD_mat_',dd)
  save(list = obj_name, file = paste0('GBD_mat_',dd,'.RData'))
  rm(list = obj_name)
  gc()
}
save(GBD_mat, file = 'GBD_mat.RData')
usethis::use_data(GBD_mat, overwrite = T)
